...
class LaneDetectorNode(Node):
    ...
    def setup_web_routes(self):
        @app.route('/')
        def index():
            return '<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><style>body{margin:0;background:#222;display:flex;justify-content:center;align-items:center;height:100vh;}img{width:640px;height:240px;object-fit:contain;border-radius:8px;}</style></head><body><img src="/video_feed"></body></html>'
    ...
    def process_frame(self, roi_img):
        ...
        if left_detected and right_detected:
            ...
            cv2.putText(display_img, f"Dynamic Offset Updated: {self.dynamic_virtual_offset}", (10, 145), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        elif left_detected:
            lane_center = left_bottom_x + self.dynamic_virtual_offset
            cv2.putText(display_img, f"Using Dynamic Offset (Left only): +{self.dynamic_virtual_offset}", (10, 145), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
        elif right_detected:
            lane_center = right_bottom_x - self.dynamic_virtual_offset
            cv2.putText(display_img, f"Using Dynamic Offset (Right only): -{self.dynamic_virtual_offset}", (10, 145), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
        else:
            lane_center = roi_W // 2
    ...
    def timer_callback(self):
        ...
        ret, buffer = cv2.imencode('.jpg', processed_img, [int(cv2.IMWRITE_JPEG_QUALITY), 70])
        if ret:
            with self.frame_lock:
