...
lane_detector_node.py가 라인트레이싱과 같은 카메라 프레임을 이 모듈에도
넘겨줍니다. 카메라를 새로 열 필요 없이, detect_signs(frame) 함수 안의 TODO만
실제 추론 코드로 채우면 자동으로 연결됩니다.
"""
...
def detect_signs(frame) -> Optional[SignDetectionResult]:
    ...
    지금은 항상 None을 반환해서 lane_detector_node.py가 이 결과를 무시하게 한다.
    """
    return None
