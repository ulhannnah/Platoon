...
> **FAQ: 왜 V2X로 받은 앞차 속도·거리를 STM32에 직접 안 보내나요?**
> 이미 RPi가 그 계산을 끝낸 값이 `target_speed_mmps`이기 때문입니다.
> `platoon_fsm.py`의 `v_target = v_리더 + K_d(d - d_target) + K_v(v_리더 - v_자차)`
> (§20.2)가 V2X 리더 속도 + UWB 차간거리를 이미 하나의 목표속도로 합쳐서 계산해둡니다.
> STM32에 그 원본 값(리더 속도, 목표거리)을 또 보내서 STM32가 다시 합치게 하면:
> - 같은 계산식(K_d, K_v)이 파이썬과 C 두 곳에 중복돼서 한쪽만 고치면 어긋남
> - K_d/K_v 튜닝이 파이썬 상수 수정에서 STM32 펌웨어 재빌드·재플래싱으로 바뀜
> - RPi가 계산한 안전 제한(MAX_CLOSING_SPEED, MAX_SPEED_STEP 등, §19.5)이 무의미해짐
>
> 그래서 이 경계는 유지합니다: **RPi가 "얼마나 빠르게"까지 다 계산해서 `target_speed`
> 하나로 내려주고, STM32는 엔코더 피드백으로 그 속도를 실제로 맞추는 것(PID)만
> 합니다.** 초음파(`front_distance_mm`)는 예외입니다 — 이건 STM32 자신에 물리적으로
> 연결된 센서라 RPi를 거칠 필요 없이 STM32가 직접 근접정지에 써도 됩니다(§5.3).
